#!/usr/bin/env python3
"""
Very Conservative Image Optimization Script using TinyPNG API
Only optimizes extremely large files with minimal quality impact.
"""
...
MIN_SIZE_TO_OPTIMIZE = 500 * 1024  # 500KB - only very large files
MAX_REDUCTION_PERCENT = 20  # Don't apply if reduction would be more than 20%
...
def optimize_image(file_path, api_key):
    """Optimize a single image using TinyPNG API with very conservative settings"""
    try:
        ...
        # Skip files that are not large enough
        if original_size < MIN_SIZE_TO_OPTIMIZE:
            print(f"  ⏭️  Skipping - file is not large enough (< {MIN_SIZE_TO_OPTIMIZE / 1024:.0f}KB)")
        ...
        print(f"  🔄 Optimizing very large file...")
...
def main():
    """Main function to optimize all project tile images very conservatively"""
    print("=== Very Conservative Image Optimization ===")
    ...
    print("This will very conservatively optimize only extremely large files.")
    print("Only files larger than 500KB with minimal quality impact will be optimized.")
    ...
    print("Very conservative optimization complete!")
