#!/usr/bin/env python3
"""
Image Optimization Script using TinyPNG API - QUALITY PRESERVING VERSION
Uses quality settings to preserve image quality while optimizing file sizes.
"""
...
# Quality settings - higher values preserve more quality
QUALITY_SETTINGS = {
    'png': 85,  # PNG quality (0-100)
...
def optimize_image(file_path, api_key):
    """Optimize a single image using TinyPNG API with quality preservation"""
    try:
        # Read the image file
        ...
        # Get file extension for quality settings
        file_ext = file_path.suffix.lower()
        quality = QUALITY_SETTINGS.get(file_ext, 85)
        ...
        if response.status_code == 201:
            ...
            # Check if the reduction is too aggressive (more than 40%)
            if savings_percent > 40:
                print(f"  ⚠️  Warning: {savings_percent:.1f}% reduction might affect quality")
                print(f"  Consider using higher quality settings if needed")
...
def main():
    ...
    print("=== Project Tile Image Optimization (Quality Preserving) ===")
    ...
    print("This will overwrite all image files with optimized versions.")
    print("Quality settings will preserve more image quality while still optimizing.")
