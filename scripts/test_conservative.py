#!/usr/bin/env python3
"""
Test script for conservative optimization
"""
...
def test_conservative_optimization():
    """Test the conservative optimization approach"""
    print("=== Testing Conservative Optimization ===")
    ...
    for file_path in project_tiles_dir.iterdir():
        if file_path.is_file() and file_path.suffix.lower() == '.png':
            if file_path.stat().st_size > MIN_SIZE_TO_OPTIMIZE:
                test_file = file_path
                break
    
    if not test_file:
        print("No large PNG files found to test with.")
        return
    ...
    if original_size < MIN_SIZE_TO_OPTIMIZE:
        print(f"File is too small to optimize (< {MIN_SIZE_TO_OPTIMIZE / 1024:.0f}KB)")
        return
...
if __name__ == "__main__":
    success = test_conservative_optimization()
    if success:
        print("\n✓ Conservative optimization test completed!")
    else:
        print("\n✗ Conservative optimization test failed!") 